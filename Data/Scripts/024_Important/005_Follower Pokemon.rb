...
ALWAYS_ANIMATED_EXCEPTION=[]
#Removed for now
#REGULAR_WALKING = true
WATERPOKEMONCANSURF = false  # Allow water pokemon to follow the player while surfing
WATERPOKEMONCANDIVE = false #Allow water pokemon to follow the player while diving
ALWAYS_ANIMATED_CAN_SURF = false #Pokemon that are always animated can follow you
...
def pbPokeStep
  for event in $game_map.events.values
    if event.name=="Poke"
      pbMoveRoute(event,[PBMoveRoute::StepAnimeOn]) if !PBTerrain.isIce?(pbGetTerrainTag)
    end
  end
end
#===============================================================================
...
#===============================================================================
            ...
            pbMessage(_INTL("{1} seems to be holding something.",e.name))
            pbPokemonFound(getConst(PBItems,items[random2]))
...
#===============================================================================
            ...
            random3=rand(3)
            if random3==0
              pbMessage(_INTL("{1} seems highly interested in the trees.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} seems to enjoy the buzzing of the bug Pokémon.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} is jumping around restlessly in the forest.",e.name,$Trainer.name))
            end
            ...
            random3=rand(3)
            if random3==0
              pbMessage(_INTL("{1} is touching some kind of switch.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} has a cord in its mouth!",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} seems to want to touch the machinery.",e.name,$Trainer.name))
            end
            ...
            random3=rand(3)
            if random3==0
              pbMessage(_INTL("{1} is sniffing around the room.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} noticed {2}'s mom is nearby.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} seems to want to settle down at home.",e.name,$Trainer.name))
            end
#===============================================================================
...
#===============================================================================
            ...
            case messages
            when 0
              pbMessage(_INTL("{1} seems to be reminiscing on all it has learned in the last year.",e.name,$Trainer.name))
            when 1
              pbMessage(_INTL("{1} seems glad to be spending its birthday with {2}.",e.name,$Trainer.name))
            when 2
              pbMessage(_INTL("{1} is having the best birthday ever!",e.name,$Trainer.name))
            when 3
              pbMessage(_INTL("{1} can't believe its been a whole year already!",e.name,$Trainer.name))
            when 4
              pbMessage(_INTL("{1} seems to be sniffing around for presents...",e.name,$Trainer.name))
            when 5
              pbMessage(_INTL("{1} seems a bit out of breath...\nYou're getting old!",e.name,$Trainer.name))
            when 6
              pbMessage(_INTL("{1} looks ready to party!",e.name,$Trainer.name))
            when 7
              pbMessage(_INTL("You wish {1} a happy birthday.\nHappy birthday, {1}!",e.name,$Trainer.name))
            when 8
              pbMessage(_INTL("{1} seems to be looking forward to another year with {2}.",e.name,$Trainer.name))
            when 9
              pbMessage(_INTL("{1} wants to eat some cake!",e.name,$Trainer.name))
            end
#===============================================================================
...
#===============================================================================
            ...
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} seems to want to play with {2}.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} is singing and humming.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} is looking up at the sky.",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} swayed and danced around as it pleased.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} is pulling out the grass.",e.name,$Trainer.name))
            end
            ...
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} let out a roar!",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} is making a face like it's angry!",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} seems to be angry for some reason.",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} chewed on your feet.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} is trying to be intimidating.",e.name,$Trainer.name))
            end
            ...
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} is looking down steadily.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} is sniffing at the floor.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} is concentrating deeply.",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} faced this way and nodded.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} is glaring straight into {2}'s eyes.",e.name,$Trainer.name))
            end
            ...
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} began poking you in the stomach.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} looks very happy.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} happily cuddled up to you.",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} is so happy that it can't stand still.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} looks like it wants to lead!",e.name,$Trainer.name))
            end
            ...
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} suddenly started walking closer.",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("Woah! {1} suddenly hugged {2}.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} is rubbing up against you.",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} is keeping close to {2}.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} blushed.",e.name,$Trainer.name))
            end

          elsif random1==5 # No Emoji
            random3=rand(5)
            if random3==0
              pbMessage(_INTL("{1} spun around in a circle!",e.name,$Trainer.name))
            elsif random3==1
              pbMessage(_INTL("{1} let our a battle cry.",e.name,$Trainer.name))
            elsif random3==2
              pbMessage(_INTL("{1} is on the lookout!",e.name,$Trainer.name))
            elsif random3==3
              pbMessage(_INTL("{1} is standing patiently.",e.name,$Trainer.name))
            elsif random3==4
              pbMessage(_INTL("{1} is looking around restlessly.",e.name,$Trainer.name))
            end
#===============================================================================
...
#===============================================================================
            ...
            if e.happiness>0 && e.happiness<=50
              $scene.spriteset.addUserAnimation(Emo_Hate, pos_x, pos_y-2)
              pbWait(70)
              pbMessage(_INTL("{1} hates to travel with {2}.",e.name,$Trainer.name))
            elsif e.happiness>50 && e.happiness<=100
              $scene.spriteset.addUserAnimation(Emo_Normal, pos_x, pos_y-2)
              pbWait(100)
              pbMessage(_INTL("{1} is still unsure about traveling with {2} is a good thing or not.",e.name,$Trainer.name))
            elsif e.happiness>100 && e.happiness<150
              $scene.spriteset.addUserAnimation(Emo_Happy, pos_x, pos_y-2)
              pbMessage(_INTL("{1} is happy traveling with {2}.",e.name,$Trainer.name))
            elsif e.happiness>=150
              $scene.spriteset.addUserAnimation(Emo_love, pos_x, pos_y-2)
              pbWait(70)
              pbMessage(_INTL("{1} loves traveling with {2}.",e.name,$Trainer.name))
            end
          end
        else
        end
...
#===============================================================================
# * Auto add Script to pbCanUseHiddenMove, fix HM bug
...
def pbPokemonFollow(x)
  pbAddDependency2(x, "Dependent", Unused_Common_Event)
  $PokemonTemp.dependentEvents.refresh_sprite
...
class DependentEvents
  def pbPokemonFound(item,quantity=1,plural=nil)
    ...
    if $PokemonBag.pbStoreItem(item,quantity)
      ...
      if pbGetItemData(item,ITEM_FIELD_USE)==3 || pbGetItemData(item,ITEM_FIELD_USE)==4
        pbMessage(_INTL("\\me[Item get]{1} found {2}!\\se[itemlevel]\\nIt contained {3}.\\wtnp[30]",e,itemname,PBMoves.getName($ItemData[item][ITEMMACHINE])))
        pbMessage(_INTL("{1} put the {2}\r\nin the {3} Pocket.",$Trainer.name,itemname,PokemonBag.pocketNames()[pocket]))
      elsif PBItems.const_defined?(:LEFTOVERS) && isConst?(item,PBItems,:LEFTOVERS)
        pbMessage(_INTL("\\me[Item get]{1} found some {2}!\\se[itemlevel]\\wtnp[30]",e,itemname))
        pbMessage(_INTL("{1} put the {2}\r\nin the {3} Pocket.",$Trainer.name,itemname,PokemonBag.pocketNames()[pocket]))
      else
        if quantity>1
          if plural
            pbMessage(_INTL("\\me[Item get]{1} found {2} {3}!\\se[itemlevel]\\wtnp[30]",e,quantity,plural))
            pbMessage(_INTL("{1} put the {2}\r\nin the {3} Pocket.",$Trainer.name,plural,PokemonBag.pocketNames()[pocket]))
          else
            pbMessage(_INTL("\\me[Item get]{1} found {2} {3}s!\\se[itemlevel]\\wtnp[30]",e,quantity,itemname))
            pbMessage(_INTL("{1} put the {2}s\r\nin the {3} Pocket.",$Trainer.name,itemname,PokemonBag.pocketNames()[pocket]))
          end
        else
          pbMessage(_INTL("\\me[Item get]{1} found one {2}!\\se[itemlevel]\\wtnp[30]",e,itemname))
          pbMessage(_INTL("{1} put the {2}\r\nin the {3} Pocket.",$Trainer.name,itemname,PokemonBag.pocketNames()[pocket]))
        end
      end
      return true
      ...
      if pbGetItemData(item,ITEM_FIELD_USE)==3 || pbGetItemData(item,ITEM_FIELD_USE)==4
        pbMessage(_INTL("{1} found {2}!\\wtnp[20]",e,itemname))
      elsif PBItems.const_defined?(:LEFTOVERS) && isConst?(item,PBItems,:LEFTOVERS)
        pbMessage(_INTL("{1} found some {2}!\\wtnp[20]",$Trainer.name,itemname))
      else
        if quantity>1
          if plural
            pbMessage(_INTL("{1} found {2} {3}!\\wtnp[20]",e,quantity,plural))
          else
            pbMessage(_INTL("{1} found {2} {3}s!\\wtnp[20]",$Trainer.name,quantity,itemname))
          end
        else
          pbMessage(_INTL("{1} found one {2}!\\wtnp[20]",e,itemname))
        end
      end
      pbMessage(_INTL("Too bad... The Bag is full..."))
      return false
    end
  end
end
...
class Scene_Map
  ...
  def update
    ...
    if $game_switches[Following_Activated_Switch] && $Trainer.party.length>0
      ...
      if (ALWAYS_ANIMATE || Input.dir4!=0 || $Trainer.party[0].hasType?(:FLYING) ||
        isConst?($Trainer.party[0].ability,PBAbilities,:LEVITATE) ||
        ALWAYS_ANIMATED_FOLLOWERS.include?($Trainer.party[0].species)) &&
...
def pbStartOver(gameover=false)
  ...
  if $PokemonGlobal.pokecenterMapId && $PokemonGlobal.pokecenterMapId>=0
    if gameover
      pbMessage(_INTL("\\w[]\\wm\\c[8]\\l[3]After the unfortunate defeat, {1} scurried to a Pokémon Center.",$Trainer.name))
    else
      pbMessage(_INTL("\\w[]\\wm\\c[8]\\l[3]{1} scurried to a Pokémon Center, protecting the exhausted and fainted Pokémon from further harm.",$Trainer.name))
    end
    pbCancelVehicles
  #  pbRemoveDependencies if !$game_switches[Following_Activated_Switch]
    ...
    if gameover
      pbMessage(_INTL("\\w[]\\wm\\c[8]\\l[3]After the unfortunate defeat, {1} scurried home.",$Trainer.name))
    else
      pbMessage(_INTL("\\w[]\\wm\\c[8]\\l[3]{1} scurried home, protecting the exhausted and fainted Pokémon from further harm.",$Trainer.name))
    end
    if homedata
      pbCancelVehicles
    #  pbRemoveDependencies() if !$game_switches[Following_Activated_Switch]
...
ItemHandlers::UseInField.add(:ESCAPEROPE,proc{|item|
   ...
   pbMessage(_INTL("{1} used the {2}.",$Trainer.name,PBItems.getName(item)))
   pbFadeOutIn(99999){
      pbCancelVehicles
...
module ItemHandlers
  def self.triggerUseOnPokemon(item,pkmn,scene)
    ...
    ret = UseOnPokemon.trigger(item,pkmn,scene)
    $PokemonTemp.dependentEvents.refresh_sprite(false) if ret
    return ret
  end
end

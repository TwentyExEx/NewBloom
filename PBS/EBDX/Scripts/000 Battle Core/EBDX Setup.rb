...
module EliteBattle
    ...
    for section in metrics.keys
      ...
      # configure player databox
        ...
        for key in metrics[section].keys
          # primary section
          if key == "__pk__"
            options[:X] = metrics[section][key]['XY'][0] if metrics[section][key]['XY']
            options[:Y] = metrics[section][key]['XY'][1] if metrics[section][key]['XY']
  ...
  def self.setupData_Pokemon
    ...
    for key in metrics.keys
      ...
      # databox config
      if metrics[key]["DATABOX"]
        options = {}
        for set in ['ShowHP', 'ExpBarWidth', 'HPBarWidth', 'Bitmap', 'HPColors', 'Container']
          options[set.upcase.to_sym] = metrics[key]["DATABOX"][set][0] if metrics[key]["DATABOX"][set]
        end
        if options.keys.length > 0
          args.push(:DATABOX_METRICS); args.push(options)
        end
      end
      # push arguments

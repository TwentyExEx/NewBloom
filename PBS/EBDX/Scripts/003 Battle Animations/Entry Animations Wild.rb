...
class EliteBattle_BasicWildAnimations
  ...
  def regiIndex?
    adj = []
    for poke in [:REGIROCK,:REGISTEEL,:REGICE,:REGIGIGAS]
      adj.push(EliteBattle.const(poke))
    end
...
class SunMoonSpeciesTransitions
  ...
  def initialize(*args)
    ...
    for j in 0...24
      ...
      str = "Graphics/EBDX/Transitions/Species/particle#{n}#{@species}"
    ...
    str = "#{@species}" if !pbResolveBitmap("Graphics/EBDX/Transitions/species#{str}") # fallback
    # initializes the necessary Pokemon graphic
    @sprites["poke1"] = Sprite.new(@viewport)
    @sprites["poke1"].bitmap = pbBitmap("Graphics/EBDX/Transitions/species#{str}")
    ...
    @sprites["poke2"].bitmap = pbBitmap("Graphics/EBDX/Transitions/species#{str}")

...
class EliteBattle_BasicTrainerAnimations
  def initialize(viewport, battletype, foe)
    ...
    # plays team skull animation if applicable
      ...
      return self.teamSkull(@viewport)
  ...
  def evilTeam(viewport=nil,trainerid=-1)
    ...
    @sprites["bg"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/background")
    @sprites["bg"].color = Color.black
    # creates background swirl
    @sprites["bg2"] = Sprite.new(@viewport)
    @sprites["bg2"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/swirl")
    ...
    for j in 0...16
      ...
      bmp = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/ray0")
    ...
    @sprites["logo"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/logo0")
    ...
    @sprites["logo"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/logo1")
    @sprites["logo"].zoom_x = 2
    @sprites["logo"].zoom_y = 2
    @sprites["logo"].z = 50
    # creates flash ring graphic
    @sprites["ring"] = Sprite.new(@viewport)
    @sprites["ring"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/ring0")
    ...
    for j in 0...32
      ...
      bmp = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/ray1")
    ...
    @sprites["ring2"].bitmap = pbBitmap("Graphics/EBDX/Transitions/EvilTeam/ring1")
  ...
  def teamSkull(viewport=nil)
    ...
    @sprites["bg"] = Sprite.new(@viewport)
    @sprites["bg"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/background")
    @sprites["bg"].color = Color.new(0,0,0,92)

    for j in 0...20
      @sprites["s#{j}"] = Sprite.new(@viewport)
      @sprites["s#{j}"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/smoke")
    ...
    @sprites["logo"] = Sprite.new(@viewport)
    @sprites["logo"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/logo")
    ...
    @sprites["shine"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/shine")
    ...
    @sprites["rainbow"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/rainbow")
    @sprites["rainbow"].center!(true)
    @sprites["rainbow"].z = 99999
    @sprites["rainbow"].opacity = 0

    @sprites["glow"] = Sprite.new(@viewport)
    @sprites["glow"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/glow")
    ...
    @sprites["burst"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/burst")
    ...
    for j in 0...24
      ...
      @sprites["p#{j}"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/particle")
    ...
    for j in 0...8
      ...
      @sprites["sl#{j}"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/paint0")
    ...
    for j in 0...12
      ...
      @sprites["sp#{j}"].bitmap = pbBitmap("Graphics/EBDX/Transitions/Skull/splat#{rand(3)}")
      @sprites["sp#{j}"].center!
    ...
    for i in 0...32
      ...
      for j in 0...16
        ...
        if @sprites["r#{j}"].opacity <= 0
          bmp = pbBitmap("Graphics/EBDX/Transitions/Skull/ray")
          w = rand(65) + 16
...
class IntegratedVSSequence
  def initialize(viewport,scene,trainerid)
    ...
    str = sprintf("vsBar%03d",@trainerid)
...
class SunMoonBattleTransitions
  ...
  def initialize(*args)
    ...
    @sprites["trainer_"].bitmap = pbBitmap(sprintf("Graphics/EBDX/Transitions/trainer%03d", @trainer.trainertype))
    # splice bitmap
  ...
  def start
    ...
    # fades in trainer
    for i in 0...12
      #@scene.commandWindow.showArrows if i < 10
      @sprites["trainer_"].tone.red -= 16
      @sprites["trainer_"].tone.green -= 16
      @sprites["trainer_"].tone.blue -= 16
    ...
    # flashes trainer
    for i in 0...10
      @sprites["trainer_"].tone.red -= 64*(i < 6 ? -1 : 1)
      @sprites["trainer_"].tone.green -= 64*(i < 6 ? -1 : 1)
      @sprites["trainer_"].tone.blue -= 64*(i < 6 ? -1 : 1)
    ...
    for i in 0...4
      @viewport.color.alpha += 32
      @sprites["trainer_"].tone.red += 64
      @sprites["trainer_"].tone.green += 64
      @sprites["trainer_"].tone.blue += 64
      self.update
      Graphics.update
    end
    4.times do
    ...
    for i in 0...8
      @viewport.color.alpha -= 32
      @sprites["trainer_"].tone.red -= 32 if @sprites["trainer_"].tone.red > 0
      @sprites["trainer_"].tone.green -= 32 if @sprites["trainer_"].tone.green > 0
      @sprites["trainer_"].tone.blue -= 32 if @sprites["trainer_"].tone.blue > 0
  ...
  def update
    ...
    @sprites["glow"].x = @sprites["trainer_"].x - @sprites["glow"].src_rect.width
...
class ClassicVSSequence
  ...
  def initialize(viewport, trainer)
    ...
    @sprites = {}
    # load trainer bitmap
    bmp = pbBitmap(sprintf("Graphics/EBDX/Transitions/classic%03d", @trainer.trainertype))
    # set up backdrop
    ...
    t = [[@trainer.name, @viewport.width - 40 - (bmp.width/2), 236, 2, Color.white, nil]]
    pbDrawTextPositions(@sprites["overlay"].bitmap, t)
    ...
    # draws the scrolling background
    @sprites["bg"] = ScrollingSprite.new(@viewport)
    str = sprintf("classicBar%03d", @trainer.trainertype)
  ...
  def start
    return if self.disposed?
    for i in 0...16
      @sprites["backdrop"].opacity += 32
      @sprites["streak"].x -= @viewport.width/8 if @sprites["streak"].x > 0
      @sprites["trainer"].x -= @viewport.width/16
      self.wait
    end
    self.wait(4)
    8.times do
      @sprites["vs"].zoom -= 1.0/8
      @sprites["vs"].opacity += 32
      self.wait
    end
    self.show
    self.wait((Graphics.frame_rate*2).to_i)
    self.finish
  end
  #-----------------------------------------------------------------------------
  #  main update for animation
  ...
  def update
    ...
    @sprites["bg"].update
    @sprites["streak"].update
    @sprites["shine"].opacity += 16 if @sprites["shine"].opacity < 255
    @sprites["shine"].angle += 8 if $PokemonSystem.screensize < 2
    @sprites["shine"].zoom_x -= 0.04*@sprites["shine"].toggle
    @sprites["shine"].zoom_y -= 0.04*@sprites["shine"].toggle
    @sprites["shine"].toggle *= -1 if @sprites["shine"].zoom_x <= 0.8 || @sprites["shine"].zoom_x >= 1.2

...
class PokeBattle_Scene
  ...
  def playerBattlerSendOut(sendOuts, startBattle = false) # Player sending out Pokémon
    ...
    ballbmp = pbBitmap("Graphics/EBDX/Pictures/pokeballs")
    ...
    # vector alignment
    v = startBattle ? EliteBattle.getVector(:SENDOUT) : EliteBattle.getVector(:MAIN, @battle)
    @vector.set(v)
    ...
    posX = (startBattle && !DISABLE_SCENE_MOTION) ? [80, 30] : [100, 40]
    posY = (startBattle && !DISABLE_SCENE_MOTION) ? [40, 160, 120] : [70, 170, 120]
    ...
    heavy = sendoutScreenShake(sendOuts, dig, startBattle, alt, metrics, dust)

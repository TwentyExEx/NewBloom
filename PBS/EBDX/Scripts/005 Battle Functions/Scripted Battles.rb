...
class PokeBattle_Scene
  ...
  def pbShowOpponent(index = 0, speech = false, file = nil)
    ...
    trainerfile = @battle.opponent ? pbTrainerSpriteFile(@battle.opponent[index].trainertype) : nil
    ...
    x = v > 0 ? -8 : -6
    20.times do
      moveEntireScene(x, (speech ? +2 : -1), true, true)
      @sprites["opponent"].opacity += 12.8
      @sprites["opponent"].x += x
      @sprites["opponent"].y += (speech ? 2 : -1)
      @sprites["box1"].zoom_x += @viewport.width/16 if speech
      @sprites["box2"].zoom_x += @viewport.width/16 if speech
      self.wait(1, true)
    end
  end
  #-----------------------------------------------------------------------------

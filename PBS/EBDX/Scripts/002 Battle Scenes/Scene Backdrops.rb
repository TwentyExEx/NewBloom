...
class PokeBattle_Scene
  ...
  def loadBackdrop
    ...
    unless [0, 1].include?(environ)
      try = EliteBattle.getData(environ, PBEnvironment, :BACKDROP)
      data = try.clone if !try.nil?
    end
    # applies additional terrain data
    try = EliteBattle.getData(terrain, PBTerrain, :BACKDROP)
    if !try.nil?
      for key in try.keys
        data[key] = try[key]
      end
    ...
    if data.has_key?("tallGrass") && @battle.doublebattle?
      data["tallGrass"][:elements] = 5 if data["tallGrass"][:elements] > 5
    end
    # applies predefined battle backdrop for map
    backdrop = EliteBattle.getData($game_map.map_id, PBMap, :BACKDROP)
    # applies predefined battle backdrops for Trainer or Pokemon
    if @battle.opponent
      bgdrop = EliteBattle.getTrainerData(@battle.opponent[0].trainertype, :BACKDROP, @battle.opponent[0])
    else
      bgdrop = EliteBattle.getData(@battle.pbParty(1)[0].species, PBSpecies, :BACKDROP, (@battle.pbParty(1)[0].form rescue 0))
    end
    ...
    data["outdoor"] = true if !data.has_key?("outdoor") && pbGetMetadata($game_map.map_id, MetadataOutdoor) && TIME_SHADING
    # Apply graphics
    ...
    if @battle.opponent
      try = sprintf("%03d",@battle.opponent[0].trainertype)
...
class BattleSceneRoom
  ...
  def initialize(viewport, scene, data)
    ...
    @data = data
    @backup = data.clone
    @defaultvector = EliteBattle.getVector(:MAIN, @battle)
    @sunny = false
  ...
  def position
    for key in @sprites.keys
      next if key == "bg" || key == "0" || key == "void" || key.include?("w_sunny") || key.include?("w_sand")
  ...
  def updateWeather
    self.setWeather
    harsh = [getConst(PBWeather, :HEAVYRAIN), getConst(PBWeather, :HARSHSUN)].include?(@battle.pbWeather)
    # snow particles
    for j in 0...72
  ...
  def drawImg(key)
    ...
    for m in keys # interprets each parameter
      k = EliteBattle.bgHashmap(m); next if k.nil? # if parameter can be mapped
  ...
  def weatherTint?
    for wth in [:Hail, :Sun, :HarshSun]
      return true if @battle.pbWeather == getConst(PBWeather, wth)
    end
    return false
  end
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  def reconfigure(data, transition = Color.black)
    if transition.is_a?(Symbol)
      @queued = data.clone
      return EliteBattle.playCommonAnimation(transition, @scene, 0)
    end

...
class PokeBattle_Scene
  ...
  def animateScene(align = false, smanim = false, &block)
    ...
    @battle.battlers.each_with_index do |b, i|
      if b
        ...
        if !@orgPos.nil? && @idleTimer > (@lastMotion.nil? ? BATTLE_MOTION_TIMER*Graphics.frame_rate : BATTLE_MOTION_TIMER*Graphics.frame_rate*0.5) && @vector.finished? && !@safaribattle
          @vector.inc = 0.005*(rand(4)+1)
          a = EliteBattle.randomCamera?(@battle, @lastMotion)
  ...
  def moveEntireScene(x=0, y=0, lock=true, bypass=false, except=nil)
    return if !bypass && DISABLE_SCENE_MOTION
    for i in 0...4
  ...
  #-----------------------------------------------------------------------------
  def setVector(*args)
    return if DISABLE_SCENE_MOTION

...
class FightWindowEBDX
  ...
  def initialize(viewport = nil, battle = nil, scene = nil)
    ...
    for i in 0...4
      pbDrawOutlineText(@typeInd.bitmap,0,24*i,192,24,eff[i],Color.white,Color.black,1)
    end
  ...
  def applyMetrics
    ...
    d2 = EliteBattle.getData(:FIGHTMENU, PBMetrics, :METRICS)
  ...
  def generateButtons
    ...
    @button = {}
    for i in 0...@nummoves
      movedata = EBMoveData.new(@moves[i].id)
      move = @moves[i]
      @button["#{i}"] = Sprite.new(@viewport)
      @button["#{i}"].param = movedata.category
      @button["#{i}"].z = 102
      @button["#{i}"].bitmap = Bitmap.new(198*2,74)
      @button["#{i}"].bitmap.blt(0,0,@buttonBitmap,Rect.new(0,move.type*74,198,74))
      @button["#{i}"].bitmap.blt(198,0,@buttonBitmap,Rect.new(198,move.type*74,198,74))
      @button["#{i}"].bitmap.blt(65,48,@catBitmap,Rect.new(0,movedata.category*22,38,22))
      @button["#{i}"].bitmap.blt(3,48,@typeBitmap,Rect.new(0,move.type*22,72,22))
      baseColor = @buttonBitmap.get_pixel(5,32+(@moves[i].type*74)).darken(0.4)
      pbSetSmallFont(@button["#{i}"].bitmap)
      pbDrawOutlineText(@button["#{i}"].bitmap,198,0,196,42,"#{move.name}",Color.white,baseColor,1)
      pp = "#{move.pp}/#{move.totalpp}"
      pbDrawOutlineText(@button["#{i}"].bitmap,0,44,191,26,pp,Color.white,baseColor,2)
      pbSetSystemFont(@button["#{i}"].bitmap)
      text = [[move.name,99,12,2,baseColor,Color.new(0,0,0,24)]]

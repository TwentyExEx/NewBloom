...
class PartyLineupEBDX
  ...
  def update
    ...
    # animate appearing
    if @toggle
      self.x += (@partyBar.width/16) * (@side%2 == 0 ? -1 : 1)
      # rotate icon balls
      for k in 0...@num
        @sprites["partyLine_#{@side}_#{k}"].angle -= (360/16) * (@side%2 == 0 ? -1 : 1)
        @sprites["partyLine_#{@side}_#{k}"].angle = 0 if @sprites["partyLine_#{@side}_#{k}"].angle >= 360 || @sprites["partyLine_#{@side}_#{k}"].angle <= -360
      end
    # animate removal
    else
      @sprites["partyLine_#{@side}"].zoom_x += 1.0/16
      @sprites["partyLine_#{@side}"].opacity -= 24
      # rotate icon balls
      for k in 0...@num
        m = @side%2 == 0 ? -k : (@num - k)
        @sprites["partyLine_#{@side}_#{k}"].angle -= (360/16) * (@side%2 == 0 ? -1 : 1)
        @sprites["partyLine_#{@side}_#{k}"].angle = 0 if @sprites["partyLine_#{@side}_#{k}"].angle >= 360 || @sprites["partyLine_#{@side}_#{k}"].angle <= -360
        @sprites["partyLine_#{@side}_#{k}"].opacity -= 24
        @sprites["partyLine_#{@side}_#{k}"].x += ((@partyBar.width/16) * (@side%2 == 0 ? -1 : 1)) - m
      end
    end
  end
  #-----------------------------------------------------------------------------

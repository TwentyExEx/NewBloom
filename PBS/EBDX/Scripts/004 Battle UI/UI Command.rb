...
class PokeBattle_Scene
  ...
  def pbCommandMenuEx(idxBattler, texts, mode = 0)
    ...
    # reset vector
    if @ret > 0
      @vector.set(EliteBattle.getVector(:MAIN, @battle))
      @vector.inc = 0.2
    end
    # unselect databoxes
    self.pbDeselectAll
...
class CommandWindowEBDX
  ...
  def applyMetrics
    ...
    d2 = EliteBattle.getData(:COMMANDMENU, PBMetrics, :METRICS)
  ...
  def refreshCommands(index)
    ...
    for i in 0...cmds.length
      ...
      x = (@safaribattle || (poke.isShadow? && poke.inHyperMode?) || i > 3) ? w : 0
      for j in 0...2
        ...
        pbDrawOutlineText(@sprites["b#{i}"].bitmap,@btnEmp.width*j,0,@btnEmp.width,h,cmds[i],Color.white,c,1)
      end
  ...
  def compileCommands(index)
    ...
    # looks up globally defined settings
    d1 = EliteBattle.getData(:BATTLE_COMMANDS, PBMetrics, :METRICS) if d1.nil?
    # array containing the default commands
    default = [_INTL("FIGHT"), _INTL("BAG"), _INTL("PARTY"), _INTL("RUN")]
    default.push(_INTL("DEBUG")) if $DEBUG && default.length == 4 && SHOW_DEBUG_FEATURES
    for i in 0...default.length
      val = default[i]; val = _INTL("CALL") if default[i] == _INTL("RUN") && (poke.isShadow? && poke.inHyperMode?)
      if !d1.nil?
  ...
  def drawLineup
    ...
    for i in 0...6
      o=3
      if i < @battle.pbParty(0).length && @battle.pbParty(0)[i]
        if @battle.pbParty(0)[i].hp <=0 || @battle.pbParty(0)[i].isEgg?
          o=2
        elsif @battle.pbParty(0)[i].status > 0
          o=1
        else
          o=0
        end
    ...
    for i in 0...6
      ...
      o=3
      if enemyindex < @battle.pbParty(1).length && @battle.pbParty(1)[enemyindex]
        if @battle.pbParty(1)[enemyindex].hp <=0 || @battle.pbParty(1)[enemyindex].isEgg?
          o=2
        elsif @battle.pbParty(1)[enemyindex].status > 0
          o=1
        else
          o=0
        end
